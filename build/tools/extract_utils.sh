...
function setup_vendor() {
    ...
    export DOT_ROOT="$3"
    if [ ! -d "$DOT_ROOT" ]; then
        echo "\$DOT_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$DOT_ROOT/$OUTDIR" ]; then
        mkdir -p "$DOT_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$DOT_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$DOT_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$DOT_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$DOT_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$DOT_ROOT"/prebuilts/tools-dot/common/smali/baksmali.jar
        export SMALIJAR="$DOT_ROOT"/prebuilts/tools-dot/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$DOT_ROOT"/prebuilts/tools-dot/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$DOT_ROOT"/prebuilts/tools-dot/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if grep "classes.dex" "$DOT_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$DOT_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$DOT_ROOT"/vendor/dot/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$DOT_ROOT"/"$OUTDIR"/radio
